...
deserializeGame str = Game getBoard (readBStr startPos) (readBStr line) 
    where
        (startPos:line:board) = BStr.lines $ BStr.filter (/= '\r') str
...
playerIsInValidState :: Player -> Game -> Bool
playerIsInValidState player (Game board _ _) = maybe False (Empty ==) cell
    where 
        (x, y) = position player
        cell = safeGet x y board
